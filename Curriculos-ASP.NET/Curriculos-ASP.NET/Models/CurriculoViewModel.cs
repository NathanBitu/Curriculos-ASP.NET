namespace Curriculos_ASP.NET.Models
{
    public class CurriculoViewModel
    {
        public string nome { get; set; }
        public string cpf { get; set; }
        public string endereco { get; set; }
        public string telefone { get; set; }
        public float pretencaoSalarial { get; set; }
        public string cargoPretendido { get; set; }
        public string instituicaoFormacao1 { get; set; }
        public string tipoFormacao1 { get; set; }
        public string formacao1 { get; set; }
        public string instituicaoFormacao2 { get; set; }
        public string tipoFormacao2 { get; set; }
        public string formacao2 { get; set; }
        public string instituicaoFormacao3 { get; set; }
        public string tipoFormacao3 { get; set; }
        public string formacao3 { get; set; }
        public string instituicaoFormacao4 { get; set; }
        public string tipoFormacao4 { get; set; }
        public string formacao4 { get; set; }
        public string instituicaoFormacao5 { get; set; }
        public string tipoFormacao5 { get; set; }
        public string formacao5 { get; set; }
        public string empresaExperiencia1 { get; set; }
        public string tipoExperiencia1 { get; set; }
        public string experiencia1 { get; set; }
        public string empresaExperiencia2 { get; set; }
        public string tipoExperiencia2 { get; set; }
        public string experiencia2 { get; set; }
        public string empresaExperiencia3 { get; set; }
        public string tipoExperiencia3 { get; set; }
        public string experiencia3 { get; set; }
        public string idioma1 { get; set; }
        public string nivelIdioma1 { get; set; }
        public string idioma2 { get; set; }
        public string nivelIdioma2 { get; set; }
        public string idioma3 { get; set; }
        public string nivelIdioma3 { get; set; }
    }
}
